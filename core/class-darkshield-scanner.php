    ...
    private function store_results( $results ) {
        ...
        foreach ( $results as $r ) {
            $exists = $wpdb->get_var( $wpdb->prepare(
                ...
                $r['domain'], $r['file_path'], $r['line_number']
            ) );
            if ( $exists ) {
                continue;
            }
        ...
        foreach ( $batch as $item ) {
            ...
            while ( $offset < 5000 ) {
                $rows = $wpdb->get_results( $wpdb->prepare(
                    "SELECT {$item['id_col']} as rid, {$item['column']} as content FROM {$item['table']} WHERE {$item['column']} LIKE %s OR {$item['column']} LIKE %s LIMIT 200 OFFSET %d",
                    '%http://%', '%https://%', $offset
                ) );
                if ( empty( $rows ) ) {
                    break;
