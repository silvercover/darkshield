    ...
    <div style="margin-top:20px;">
            ...
            <?php if ($tp > 1) : ?>
                <div class="tablenav bottom" style="margin-top:10px;">
                    <div class="tablenav-pages">
                        ...
                        // paginate_links returns safe HTML, use wp_kses_post
                        echo wp_kses_post($pagination);
                        ?>
                        <span class="displaying-num" style="margin-left:10px;">
                            <?php  /* translators: %1$s: start number, %2$s: end number, %3$s: total entries */
                            printf(
                                esc_html__('%1$s–%2$s of %3$s', 'darkshield'),
                                esc_html(number_format_i18n($off + 1)),
                                esc_html(number_format_i18n(min($off + $pp, $total))),
        ...
        <?php else : ?>
            <div class="notice notice-error">
                <p><?php esc_html_e('Log table missing. Deactivate and reactivate the plugin.', 'darkshield'); ?></p>
            </div>
        <?php endif; ?>
        ...
        <?php if ($ok && $st > 0) : ?>
            <div class="card" style="max-width:100%;padding:15px;margin-top:20px;">
                <div style="display:flex;gap:20px;flex-wrap:wrap;font-size:13px;color:#666;">
                    <span><strong><?php esc_html_e('Retention:', 'darkshield'); ?></strong>
                        <?php
                        /* translators: %d: number of days for log retention */
                        printf(esc_html__('%d days', 'darkshield'), DarkShield_Utils::get_setting('log_retention', 30)); ?>
                    </span>
